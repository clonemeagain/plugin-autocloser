...
LIMIT %d", TICKET_TABLE, $age_days, $from_status, $whereFilter, $max);
    ...
    function postReply($ticket, $new_status, $admin_reply)
    {
        ...
        foreach ($options as $option => $method) {
            if (strpos($admin_reply, $option) !== FALSE) {
                ...
                if (! $variables[$option]) {
                    error_log("No output for $method.. shit.");
                } else {
                    error_log("YAY, $method spewed out: " . strlen($variables[$option]));
                    // print_r($variables[$option]);
                }
            }
        }
    ...
    private function fetchFirstResponse(Ticket $ticket)
    {
        ...
        if ($response instanceof AnnotatedModel) {
            if ($response->get('format') == 'text') {
                return "<br /><hr /><h3>{$response->get('title')}</h3><pre>{$response->get('body')}</pre>";
            }
            // Make prettier?
            return "<br /><hr /><h3>{$response->get('title')}</h3><p>{$response->get('body')}</p>";
        }
    }
        ...
        foreach ($ticket->getClientThread()->all() as $entry) {
            if ($entry instanceof AnnotatedModel) {
                ...
                if ($entry->get('format') == 'text') {
                    $msg .= "<br /><hr /><h3>{$entry->get('title')}</h3><pre>{$entry->get('body')}</pre>";
                } else {
                    // Make prettier?
                    $msg .= "<br /><hr /><h3>{$entry->get('title')}</h3><p>{$entry->get('body')}</p>";
                }
            }
        }
        return $msg;
